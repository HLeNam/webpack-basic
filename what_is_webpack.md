...
module.exports = {
    ...
    }, // assetModuleFilename giúp định nghĩa tên file của các tài nguyên
    ...
    }, // font minh import trong file css và webpack nó tự động
};
```

-   Mình không bỏ phần mở rộng của file font vào `test` font mình import trong file css và webpack nó tự động nhận diện được. Nếu bạn muốn import font trong js thì có thể thêm phần mở rộng của font vào `test` như sau
...
> -   `[query]`: query bắt đầu với dấu `?`. Ví dụ: abc.com/bitcoin.pdf?id=abcdef, khi người đùng click vào link này sẽ xem được file pdf và server cũng nhận được một request có query string id=abcdef để thực hiện một hành động gì đó.
> -   `[fragment]`: fragment bắt đầu với đấu `#`. Ví dụ abc.com/bitcoin.pdf#page=4, khi người đùng click vào link này thì mở file pdf và chuyển ngay đến page 4. Tham khảo thêm về URI fragment [tại đây](https://en.wikipedia.org/wiki/URI_fragment)
...
> -   `[ext]`: Phần đuôi mở rộng bắt đầu với đấu . (tính năng này không có sẵn cho [output, filename](https://webpack.js.org/configuration/output/#outputfilename))

-   `[file]` = `[path][base]`
-   `[base]` = `[name][ext]`
-   Full path: `[path][name][ext][query][fragment]` = `[path][base][query]`
...
Cài `npm i webpack-bundle-analyzer -D`
